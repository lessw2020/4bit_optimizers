...
def group2d_tensor(input: torch.Tensor, gp_sz: int):
    ...
    if C1 % gp_sz != 0:
        ...
        input = torch.cat([input, torch.zeros([C0, delta], dtype=input.dtype, device=input.device)],
                                dim=1)
...
# deprecated
def sm3_quant(x, **kwargs):
    # save normal kwargs already finished
    qx = x.abs()
    max_dims = []
    for i in range(x.dim()):
        nu_max = _max_reduce_except_dim(qx, i)
        if isinstance(kwargs['sm3_history'], list):
            torch.max(kwargs['sm3_history'][i], nu_max, out=nu_max)
        max_dims.append(nu_max)
    kwargs['gen'] = (max_dims,) # not changed afterwards
    # quantize
    signed = kwargs['signed']
    b = kwargs['b']
    if b == 0:
        # NOTE: exactly SM3 algorithm
        return torch.sign(x), kwargs
    else:
        st = _compute_sm3_scale_tensor(max_dims)
        MRQ, lo, hi = prepare_quant_boundary(b, signed)
        qx = atom_quant(x, st, MRQ, lo, hi, kwargs['round_type'])
    return qx, kwargs


# deprecated
def sm3_dequant(qx, **kwargs):
    # self-consistent
    max_dims = kwargs['gen'][0]
    st = _compute_sm3_scale_tensor(max_dims)
    signed = kwargs['signed']
    dtype = kwargs['dtype']
    ...
    if b == 0:
        # NOTE: exactly SM3 algorithm
        x = st * qx if signed else st
    else:
        MRQ, lo, hi = prepare_quant_boundary(b, signed)
        x = atom_dequant(qx, st, MRQ)
    x = x.to(dtype=dtype, memory_format=memory_format)
    assert x.shape == kwargs['shape'], f"The original shape is {kwargs['shape']} the dequantized shape is {x.shape}"
    return x
...
# deprecated
def adafactor_quant(x, **kwargs):
    # normal kwargs saving already finished
    assert x.dim() == 2
    qx = x.abs()
    sum_dims = []
    for i in range(x.dim()):
        one_dim_sum = qx.sum(dim=i, keepdim=True)
        sum_dims.append(one_dim_sum)
    kwargs['gen'] = (sum_dims,) # not changed afterwards
    # quantize
    signed = kwargs['signed']
    b = kwargs['b']
    # NOTE: exactly adafactor algorithm
    return torch.sign(x), kwargs


# deprecated
def adafactor_dequant(qx, **kwargs):
    # self-consistent
    sum_dims = kwargs['gen'][0]
    st = sum_dims[0] * sum_dims[1] / sum_dims[0].sum().item()
    signed = kwargs['signed']
    dtype = kwargs['dtype']
    memory_format = kwargs['memory_format']
    b = kwargs['b']
...
def symmetric_atom_quantize(x, bit_width, res, **kwargs):
    r'''
    symmetric quantization excluding zero
    only support for signed case, with single scale.
    ========
    Parameters:
    x: zero mean tensor
    res: 
    '''
    num_points = 2 ** bit_width - 1
    translate_x = x + num_points / 2 * res
    qx = atom_quant(translate_x, res * num_points, num_points, 0, num_points, **kwargs)
    return qx


def symmetric_atom_dequantize(qx, bit_width, res, **kwargs):
    r'''
    symmetric dequantization excluding zero
    only support for signed case, with single scale.
    ========
    Parameters:
    x: zero mean tensor
    ...
    '''
    num_points = 2 ** bit_width - 1
    translate_x = atom_dequant(qx, res * num_points, num_points)
    x = translate_x - num_points / 2 * res
    return x
...
def create_exp_map(qx, b, signed):
    if signed:
        N = (2 ** (b - 1))
        logqx = qx.abs().log()
        pos = torch.exp(torch.linspace(logqx.min(), logqx.max(), N))
        neg = -pos.flip(0)
        # qmap = torch.cat([neg, torch.as_tensor([0]), pos]) # less one ver.
        qmap = torch.cat([neg, pos]) # no zero ver.
    else:
        N = (2 ** b) - 1
        logqx = qx.log()
        pos = torch.exp(torch.linspace(logqx.min(), logqx.max(), N))
        qmap = torch.cat([torch.as_tensor([0]), pos])
    return qmap


def create_log_map(b, signed):
    Z = torch.exp(torch.as_tensor([1])) - 1
    if signed:
        # positive_quantization_points = (torch.exp(torch.linspace(0, 1, 2 ** (b - 1) + 1)) - 1) / Z # no minimal ver.
        positive_quantization_points = (torch.exp(torch.linspace(0, 1, 2 ** (b - 1) + 1)) - 1)[1:] / Z # no zero ver.
        # positive_quantization_points = (torch.exp(torch.linspace(0, 1, 2 ** (b - 1))) - 1) / Z # less one ver.
        negative_quantization_points = -positive_quantization_points.flip(0)
        # qmap = torch.cat((negative_quantization_points[1:-1], positive_quantization_points)) # no minimal ver.
        qmap = torch.cat((negative_quantization_points, positive_quantization_points)) # no zero ver.
        # qmap = torch.cat((negative_quantization_points[:-1], positive_quantization_points)) # less one ver.
    else:
        # qmap = (torch.exp(torch.linspace(0, 1, 2 ** b)) - 1) / Z # default ver.
        qmap = (torch.exp(torch.linspace(0, 1, (2 ** b) + 1)) - 1)[1:] / Z # no zero ver.
    return qmap


# --- quantization enable ---
def always_bool(val: bool):
    def f(*inputs, **kwargs):
        return val
    return f


always_true = always_bool(val=True)
always_false = always_bool(val=False)


def numel_enable(th):
    def enable(p_name, state_name, x):
        return x.numel() > th if th is not None else True
    return enable


def suffix_enable(suffix, has_suffix=False):
    def enable(p_name, state_name, x):
        return not (p_name.endswith(suffix) ^ has_suffix)
    return enable


def intersect_enable(*func_list):
    def enable(p_name, state_name, x):
        for func in func_list:
            if not func(p_name, state_name, x):
                return False
        return True
    return enable


def union_enable(*func_list):
    def enable(p_name, state_name, x):
        for func in func_list:
            if func(p_name, state_name, x):
                return True
        return False
    return enable


def shape_enable(dim=2):
    def enable(p_name, state_name, x):
        return x.dim() >= dim
    return enable


def get_enable_fn_from_subconfig(subconfig):
    if not subconfig.ENABLE:
        return always_false
    func_list = []
    # if only_suffix is not None:
    #     assert isinstance(only_suffix, List) 
    #     for suffix in only_suffix:
    #         func_list.append(suffix_enable(suffix, has_suffix=True))
    #     union = union_enable(*func_list)
    #     return intersect_enable(numel_enable(th), union)
    func_list.append(numel_enable(subconfig.THRESHOLD))
    func_list.append(shape_enable())
    for suffix in subconfig.EXCLUDE_SUFFIX:
        func_list.append(suffix_enable(suffix, has_suffix=False))
    return intersect_enable(*func_list)
